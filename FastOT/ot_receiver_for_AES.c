...
//Print receiver's parameter to Receiver_Secret.txt
void print_sender_parameters(FILE *f, RECEIVER *receiver)
{
  fprintf(f, "S: ");
  record(f, (char*)(&(receiver -> S)), sizeof(receiver -> S));
  fprintf(f, "\n");
}
...
void ot_receiver_test(RECEIVER *receiver, int sockfd) {
  ...
  // Receiver_Secret.txt records all data sent or received by receiver. (in Hex
  // system)
  FILE *fp;
  ...
  // of OTs (must be multiple of 4). For the following number_of_ots lines, each
  // line contains 0 or 1 which indicates receiver's choice for each OT.
  FILE *input;
  ...
  //Print receiver's parameter to Receiver_Secret.txt
  print_sender_parameters(fp, receiver);
  ...
  for (i = 0; i < number_of_ots; i += 4) {
    ...
    if (VERBOSE) {
      for (j = 0; j < 4; j++) {
        ...
        if (cs[j] == 0)
          mydecrypt(AES_pack, keys[j], AES_KEY);
        else
          mydecrypt(AES_pack + AES_KEY_BYTES, keys[j], AES_KEY);
