...
void ot_sender_test(SENDER * sender, int newsockfd)
{
	...
	//The following 2*number_of_ots lines contains number_of_ots pairs of 16bytes AES keys.
    FILE *input;
    input = fopen("Sender_input.txt", "r");

	//Sender_Secret.txt records all data sent or received by sender. (in Hex system)
	FILE *fp;
