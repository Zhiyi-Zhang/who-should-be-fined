...
//Print sender's parameter to Sender_Secret.txt
void print_sender_parameters(FILE *f, SENDER *sender)
{
  fprintf(f, "y: ");
  record(f, (char*)(&(sender -> y)), sizeof(sender -> y));
  fprintf(f, "\nyS: ");
  record(f, (char*)(&(sender -> yS)), sizeof(sender -> yS));
  fprintf(f, "\n");
}
...
void ot_sender_test(SENDER *sender, int newsockfd) {
  ...
  //Print sender's parameter to Sender_Secret.txt
  print_sender_parameters(fp, sender);
